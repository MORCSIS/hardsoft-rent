...
function checkExistingRecords($data, $con) {
    ...
    foreach ($data as $index => $row) {
        ...
        $stmt = $con->prepare("SELECT id_articulo FROM articulos WHERE id_articulo = ? OR UPPER(nom_articulo) = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['articulos'])) {
    $file = $_FILES['articulos'];
    $response = [];
    
    if (!validateFileName($file['name'])) {
        $response = ['error' => 'El archivo debe llamarse "articulos.csv" para continuar.'];
        ...
        if (!empty($validationErrors)) {
            ...
            if (!empty($existingRecords)) {
                ...
                if (isset($_POST['confirm']) && $_POST['confirm'] === 'true') {
                    $stmt = $con->prepare("INSERT INTO articulos (id_articulo, nom_articulo) VALUES (?, ?)");
                    $insertedCount = 0;
                    ...
                    header("Location: articulos.php?success=true&count=" . $insertedCount);
                    exit;
                }
            }
        }
...
<head>
    ...
    <title>Carga Masiva de artículos</title>
...
<body>
    ...
    <div class="main-content" id="main-content">
        ...
        <form id="uploadForm" method="POST" enctype="multipart/form-data">
                <div>
                    <label for="articulos">Seleccione archivo CSV:</label>
                    <input type="file" name="articulos" id="articulos" accept=".csv" required>
                </div>

                <div class="button-group">
                    <button type="button" onclick="window.location.href='articulos.php'">Cancelar</button>
        ...
        <div id="messageContainer"></div>
            <div class="file-info">
                ...
                <p>2. La primera columna debe contener el ID de articulo (numérico)</p>
                <p>3. Considera el último ID de articulo, deben ser contínuos </p>
                <p>4. La segunda columna debe contener el nombre de articulo</p>
                <p>5. Se recomienda incluir una fila de encabezados</p>
                <p>6. Ejemplo de formato:</p>
                <pre>id_articulo,nom_articulo
1,articulo 1
2,articulo 2</pre>
            </div>
    ...
    <script>
        document.getElementById('uploadForm').addEventListener('submit', function(e) {
            ...
            fetch('articulos_carga_masiva.php', {
                method: 'POST',
                body: formData
            })
...
registerAction($id_usuario, $accion, $descripcion);
                                ?>
                            window.location.href = `articulos.php?success=true&count=${data.count}`;
                        })
                        .catch(error => {
                            uploadButton.disabled = false;
