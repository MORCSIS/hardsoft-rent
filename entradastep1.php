...
// Consulta para obtener detalles del cliente origen seleccionado
$sqlclienteO = "SELECT DISTINCT * FROM clientes WHERE id_cliente = $id_clienteOrig";
$result1 = $con->query($sqlclienteO);

// Consulta para obtener detalles del almacén receptor
$sqlclienteD = "SELECT DISTINCT * FROM clientes where id_cliente <= 101"; 
...
<body>
    ...
    <!-- Formulario para seleccionar cliente que devuelvue -->
        ...
        <h3>Paso 1: Seleccionar almacén receptor</h3>
        <form action="entradastep2.php" method="GET">
        <input type="hidden" name="id_clienteOrig" value="<?php echo htmlspecialchars($id_clienteOrig); ?>">
        <label for="cliente">Almacén que recibe:</label>
