...
// Obtener el valor del nombre del modelo, eliminar espacios y convertir a mayúsculas
$nom_modelo = mb_strtoupper(trim($_GET['nom_modelo']));

// Preparar la consulta para evitar SQL Injection
$sql = "INSERT INTO modelos (nom_modelo) VALUES (?)";
$stmt = $con->prepare($sql);

if ($stmt) {
    // Vincular parámetros, "s" significa que es una cadena (string)
    $stmt->bind_param("s", $nom_modelo);

    // Ejecutar la consulta
    if ($stmt->execute()) {
        // Si la consulta se ejecuta correctamente, mostrar una alerta de éxito
        echo '<script type="text/javascript">
                alert("Registro: ' . $nom_modelo . ' agregado correctamente");
...
registerAction($id_usuario, $accion, $descripcion);
        ...
        echo '<script type="text/javascript">
                alert("Error en la transacción: Registro duplicado  '. $nom_modelo .' ya existe, elija otro ");
                window.location.href = "modelos.php";
              </script>';
    }
